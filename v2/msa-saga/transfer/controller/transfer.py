...
class TransferController():
    ...
    def update_transfer(self, json_request):
        data = json_request
        
        if json_request['status_code'] in (200, 201):
            message = 'DONE'
            data['transfer_result'] = json_request
            status_code = json_request['status_code']
            self.model.update(json_request['data']['transaction_id'], message)
        else:
    ...
    def update_transfer_interbank(self, json_request):
        status_code = json_request['status_code']
        message = None
        data = json_request
        ...
        if status_code in (200, 201):
            ...
            data['settlement_response'] = json_request['data']
            transaction = self.model.detail(json_request['data']['transaction_id'])
            ...
            if notify_result.status_code in (200, 201):
                ...
                status_code = 500
                self.model.update(json_request['data']['transaction_id'], message)
                self.model.reversal_interbank(transaction['from_account_number'], transaction['to_account_number'], transaction['bank_code'], json_request['data']['transaction_id'], transaction['amount'], json_request['data']['journal_number'])
        else:
            message = 'SETTLEMENT_FAILED'
            status_code = 500
            self.model.update(json_request['data']['transaction_id'], message)
    ...
    def update_reversal_transfer_interbank(self, json_request):
