...
@transfer.route("/update/", methods=["POST"])
def update_transfer():
    return TransferController().update_transfer(request.get_json())

@transfer.route("/update/interbank", methods=["POST"])
def update_transfer_interbank():
    return TransferController().update_transfer_interbank(request.get_json())

@transfer.route("/update/reversal/interbank", methods=["POST"])
def update_reversal_transfer_interbank():
    return TransferController().update_reversal_transfer_interbank(request.get_json())
