...
class SimpleUser(SequentialTaskSet):
    ...
    def test_transaction_list(self):
        with self.client.get(f"/transaction/list/?cif_number={self.cif_number}&skip=0&limit=10", catch_response=True, name="n_test_transaction_list") as response:
            if response.status_code not in (200, 201):
    ...
    def test_hist_trx(self):
        with self.client.get(f"/historical_transaction/?account_number={self.from_account_number}&skip=0&limit=10", catch_response=True, name="o_test_hist_trx") as response:
            if response.status_code not in (200, 201):
