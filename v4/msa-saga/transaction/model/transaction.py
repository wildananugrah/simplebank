...
class Transaction:
    ...
    journal_ack: JournalAck = JournalAck()
    session = session
    ...
    def generate_transaction_id(self, size=10):
        try:
            ...
            db_transaction_id = self.session.query(TblTransaction).filter(TblTransaction.transaction_id == transaction_id).first()
    ...
    def save(self, transaction_type, transaction_id, from_account_number, to_account_number, to_bank_code, amount, journal_number, cif_number, status, description=""):
...
class DebitTransaction(Transaction):
    ...
    def debit(self):
        try:
            ...
            self.journal_ack.notify_journal(self.transaction_type, request_message, 'ack_debit')
