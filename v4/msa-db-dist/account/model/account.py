...
class Account:
    ...
    def detail(self, account_number):
        account = self.db_read.simplebank_db.accounts.find_one({ 'account_number' : account_number }, { '_id' : False })
        if account:
            return account
        else:
    ...
    def create(self, cif_number, currency = 'IDR', balance = 0):
        ...
        self.db_write.simplebank_db.accounts.insert_one(data, {'_id' : False}) 
        data['_id'] = None # magic happens here. 
        return data
    ...
    def update(self, account_number, current_balance):
        ...
        self.db_write.simplebank_db.accounts.update_one(query, { '$set' : new_value })

        return self.detail(account_number)

    def delete(self, account_number):
        self.detail(account_number)
        self.db_write.simplebank_db.accounts.delete_one({ 'account_number' : account_number })
        return True

    def list(self, cif_number, skip, limit):
        return list(self.db_read.simplebank_db.accounts.find({ 'cif_number' : cif_number}, { '_id' : False }).skip(int(skip)).limit(int(limit)))
